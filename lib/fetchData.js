export default async function fetchData(...args){//count, id, start_date
    ...
    //count
    if(args[0]){
        const response = await fetch(`https://api.nasa.gov/planetary/apod?count=${args[0]}&api_key=${process.env.API_KEY}`)
    ...
    //id
    if(args[1]){
        const response = await fetch(`https://api.nasa.gov/planetary/apod?date=${args[1]}&api_key=${process.env.API_KEY}`)
        if(response.status !== 200) return
        const data = await response.json()
        return data
    }
